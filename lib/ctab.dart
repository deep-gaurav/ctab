...
class _IndicatorPainter extends CustomPainter {
  ...
  double startOf(int tabIndex) {
    assert(_currentTabOffsets != null);
    assert(_currentTabOffsets!.isNotEmpty);
    assert(tabIndex >= 0);
    assert(tabIndex <= maxTabIndex);
    return _currentTabOffsets![tabIndex];
  }

  double endOf(int tabIndex) {
    assert(_currentTabOffsets != null);
    assert(_currentTabOffsets!.isNotEmpty);
    assert(tabIndex >= 0);
    assert(tabIndex <= maxTabIndex);
    return _currentTabOffsets![tabIndex + 1];
  }

  double widthOf(int tabIndex) {
    assert(_currentTabOffsets != null);
    assert(_currentTabOffsets!.isNotEmpty);
    assert(tabIndex >= 0);
    assert(tabIndex <= maxTabIndex);
    return _currentTabOffsets![tabIndex + 1] - _currentTabOffsets![tabIndex];
  }
...
///
/// {@tool dartpad}
/// This sample shows the implementation of [CTabBar] and [TabBarView] using a [DefaultTabController].
...
///
/// {@tool dartpad}
/// [CTabBar] can also be implemented by using a [TabController] which provides more options
/// to control the behavior of the [CTabBar] and [TabBarView]. This can be used instead of
...
///  * [CTabBar], which is used to display the [Tab] that corresponds to each page of the [TabBarView].
class CTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// and the entire [CTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// [CTabBar] in a [Padding] widget. When [isScrollable] is true, the scrollable itself is inset,
  ...
  /// An optional callback that's called when the [CTabBar] is tapped.
  ///
  ...
  /// How the [CTabBar]'s scroll view should respond to user input.
  ///
  ...
  /// Returns whether the [CTabBar] contains a tab with both text and icon.
  ///
  /// [CTabBar] uses this to give uniform padding to all tabs in cases where
...
class _CTabBarState extends State<CTabBar> {
  ...
  @override
  void didUpdateWidget(CTabBar oldWidget) {
  ...
  double _tabScrollOffset(
    ...
    double divw = (index + 1) * (viewportWidth / (divisions + 2));
    ...
    return (tabCenter - divw)
        .clamp(minExtent, _indicatorPainter!.startOf(index));
  }
